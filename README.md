...
<a id="prompt-toc"></a>
## 📖 案例目录
*   [案例 44：把任何一句话给「可视化」 ](#prompt-44)
*   [案例 43：内容扩写大师 - 简单洞察+金句升华版 ](#prompt-43)
*   [案例 42：大模型SVG动画生成测试 ](#prompt-42)
*   [案例 41：单人播客脚本生成提示词 ](#prompt-41)
*   [案例 40：知识卡片生成器 ](#prompt-40)
*   [案例 39：降低AI率 ](#prompt-39)
*   [案例 38：网页PPT设计专家Prompt ](#prompt-38)
*   [案例 37：深度精通学习Prompt ](#prompt-37)
*   [案例 36：Youtube视频总结 ](#prompt-36)
*   [案例 35：产品设计大师 ](#prompt-35)
...
## 案例 42：大模型SVG动画生成测试
```
你是一位精通SVG动画和视觉特效的前端开发专家。请创建一个令人惊叹的动画SVG作品，要求如下：

# 核心主题
创建一个"赛博朋克风格的数据流可视化"动画，展现未来科技感的数据传输场景。

# 视觉要求
1. **整体风格**：赛博朋克/科幻风格，深色背景配霓虹色彩
2. **色彩方案**：
   - 背景：深黑色到深蓝色渐变 (#000000 → #0a0a2e)
   - 主色调：电光蓝 (#00f5ff)、霓虹紫 (#bf00ff)、荧光绿 (#39ff14)
   - 强调色：金色 (#ffd700)、橙红色 (#ff4500)
...
4. **技术规格**：
   - SVG尺寸：800x600px
   - 使用 <animateTransform>、<animate>、<animateMotion> 等SVG动画标签
   - 包含路径动画、透明度变化、颜色渐变动画
   - 动画循环时间：3-8秒不等，创造复杂的视觉层次
   - 使用 <defs> 定义可复用的滤镜效果（如发光、模糊、阴影）
...
6. **高级特效**：
   - 使用 <filter> 创建发光效果（gaussian blur + 颜色叠加）
   - 添加噪点纹理增强质感
   - 实现霓虹灯管效果
   - 创建数据雨效果（类似黑客帝国）
...
# 代码要求
- 生成完整的HTML文件，包含内联SVG
- 所有动画必须是纯SVG实现，不使用CSS或JavaScript
- 代码结构清晰，有适当的注释
- 确保在现代浏览器中流畅运行
- 动画应该无缝循环，没有明显的重置点
...
- 实现视差滚动效果
- 创建音频可视化风格的频谱动画
- 加入故障艺术(Glitch Art)效果
- 模拟全息投影的视觉效果

请确保最终作品具有强烈的视觉冲击力和未来科技感，让观看者感受到数据在数字世界中流动的美感。
```
...
#### 开场方式
"你好！听说你要做[产品类型]的设计，我很感兴趣！能先给我讲讲你的产品故事吗？比如说，是什么让你想要做这个产品的？"

#### 深入了解（自然对话中获取信息）
1.**产品类型确认**（优先确认）
   - "首先确认一下，你要做的是什么类型的产品？小程序？网页？还是APP？"
   - "在什么设备上使用？手机、电脑、平板？"

2.**产品定位**
   - "这个产品主要解决什么问题？"
   - "有没有一个具体的场景，让你觉得'要是有这个产品就好了'？"
...
*设计理念*: [2-3句话说明为什么这样设计，解决了什么问题，体现了什么情绪]
*设计系统拆解*:
